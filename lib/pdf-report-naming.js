 ...
 */
export function buildAnalyticsReportFilename(filters, { programTitle = '', trainingPartnerName = '' } = {}) {
 ...
 */
export function buildTraineesReportFilename(filters, { programTitle = '', partnerName = '', trainerName = '' } = {}) {
    const date = new Date().toISOString().slice(0, 10);
    const parts = [
        'RETC-trainees',
        filters.year === 'all' ? 'years-all' : `year-${slugifyForFilename(String(filters.year), 8)}`,
 ...
 */
export function buildProgramsReportFilename(filters, { programTitle = '', partnerName = '', trainerName = '' } = {}) {
    const date = new Date().toISOString().slice(0, 10);
    const parts = [
        'RETC-programs',
        filters.year === 'all' ? 'years-all' : `year-${slugifyForFilename(String(filters.year), 8)}`,
