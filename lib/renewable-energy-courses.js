...
const TITLE_INFERENCE_RULES = [
    ...
    { key: 'refrigeration', test: (t) => /\brefrigerat/i.test(t) || /\bcooling\b/i.test(t) },
    { key: 'climate_change', test: (t) => /\bclimate\s*change\b/i.test(t) },
];
...
export function resolveTraineeCourseCategoryKey(trainee, programById = {}, programIdOverride = '') {
    const programId = String(programIdOverride || getProgramIdFromTrainee(trainee) || '').trim();
    let courseKey = programId
        ? getCourseKeyFromProgram(resolveProgramFromMap(programById, programId))
        : getTraineeCourseKey(trainee, programById);
...
/** Whether a trainee matches a course filter (`all` = no filter). Any enrollment counts. */
export function traineeMatchesCourseFilter(trainee, courseFilter, programById = {}) {
    if (courseFilter == null || courseFilter === '' || courseFilter === 'all')
        return true;
    const ids = trainee?.program_ids?.length
        ? trainee.program_ids
        : [getProgramIdFromTrainee(trainee)].filter(Boolean);
    return ids.some((pid) => {
        const prog = resolveProgramFromMap(programById, pid);
        return prog && getCourseKeyFromProgram(prog) === courseFilter;
    });
}
