...
export function mergeTraineeWithEnrollment(enrollmentByTrainee, trainee) {
    ...
    // Keep the original course as the primary pointer when present,
    // even after adding enrollment rows for additional courses.
    const mergedProgram = legacyProgram || String(primary?.programId || '').trim();
    const programIds = Array.from(new Set([
        ...enrollments.map((e) => String(e?.programId || '').trim()).filter(Boolean),
        legacyProgram,
    ].filter(Boolean)));
