...
import { expandTraineesByEnrollment } from '@/lib/trainee-enrollment';

import { CHART_GENDER_STACK_COLORS, CHART_LEVEL_COLORS, getChartSeriesColor } from '@/lib/chart-brand-colors';
 ...
 */
export function buildLevelByCourseCategory(trainees, programById, options = {}) {
    const matrix = {};
    ...
    const rows = options.alreadyExpanded ? trainees : expandTraineesByEnrollment(trainees);
    rows.forEach((trainee) => {
        const catKey = resolveTraineeCourseCategoryKey(trainee, programById, trainee.program_id);
...
export const LEVEL_STACK_SERIES = [
    { countKey: 'beginner', pctKey: 'beginnerPct', name: 'Beginner' },
    { countKey: 'technician', pctKey: 'technicianPct', name: 'Technician' },
    { countKey: 'trainer', pctKey: 'trainerPct', name: 'Trainer' },
    { countKey: 'unspecified', pctKey: 'unspecifiedPct', name: 'Unspecified' },
];

export const GENDER_STACK_SERIES = [
    { countKey: 'male', pctKey: 'malePct', name: 'Male' },
    { countKey: 'female', pctKey: 'femalePct', name: 'Female' },
    { countKey: 'other', pctKey: 'otherPct', name: 'Other' },
];

/** Stacked segments: gender within each participant level (e.g. Female · Beginner). */
export const LEVEL_GENDER_STACK_SERIES = (() => {
    const series = [];
    TRAINEE_LEVELS.forEach((level) => {
        GENDER_STACK_KEYS.forEach((gender) => {
            series.push({
                countKey: `${gender}_${level.key}`,
                name: `${GENDER_LABEL_BY_KEY[gender]} · ${level.label}`,
            });
        });
    });
    GENDER_STACK_KEYS.forEach((gender) => {
        series.push({
            countKey: `${gender}_unspecified`,
            name: `${GENDER_LABEL_BY_KEY[gender]} · Unspecified`,
        });
    });
    return series;
})();

export const LEVEL_GENDER_STACK_COLORS = Object.fromEntries(
    LEVEL_GENDER_STACK_SERIES.map((entry, index) => {
        const [gender, level] = entry.countKey.split('_');
        if (level === 'unspecified')
            return [entry.countKey, '#94a3b8'];
        if (gender === 'female')
            return [entry.countKey, level === 'beginner' ? '#fb923c' : level === 'technician' ? '#ff8829' : '#f97316'];
        if (gender === 'male')
            return [entry.countKey, level === 'beginner' ? '#0284c7' : level === 'technician' ? '#047857' : '#0b8d68'];
        return [entry.countKey, getChartSeriesColor(index)];
    }),
);

function emptyLevelGenderRow() {
    const row = { male: 0, female: 0, other: 0 };
    LEVEL_GENDER_STACK_SERIES.forEach(({ countKey }) => {
        row[countKey] = 0;
    });
    return row;
}

function emptyLevelTotals() {
    return { beginner: 0, technician: 0, trainer: 0, unspecified: 0 };
}

function emptyGenderTotals() {
    return { male: 0, female: 0, other: 0 };
}

function levelSummaryFromRow(row) {
    const total = row.total || 0;
    const summary = TRAINEE_LEVELS.map((l) => ({
        key: l.key,
        label: l.label,
        count: row[l.key] || 0,
        percent: total > 0 ? Math.round(((row[l.key] || 0) / total) * 1000) / 10 : 0,
        percentLabel: formatPercent(row[l.key] || 0, total, 1),
    })).filter((l) => l.count > 0);
    if ((row.unspecified || 0) > 0) {
        summary.push({
            key: 'unspecified',
            label: 'Unspecified',
            count: row.unspecified,
            percent: total > 0 ? Math.round((row.unspecified / total) * 1000) / 10 : 0,
            percentLabel: formatPercent(row.unspecified, total, 1),
        });
    }
    return summary;
}

function genderSummaryFromRow(row) {
    const total = row.total || 0;
    return GENDER_STACK_KEYS.map((key) => ({
        key,
        label: GENDER_LABEL_BY_KEY[key],
        count: row[key] || 0,
        percent: total > 0 ? Math.round(((row[key] || 0) / total) * 1000) / 10 : 0,
        percentLabel: formatPercent(row[key] || 0, total, 1),
    })).filter((g) => g.count > 0);
}

/** Narrow a level × category breakdown to one course category (or return unchanged for `all`). */
export function filterLevelBreakdownByCategory(breakdown, categoryKey) {
    if (!breakdown || !categoryKey || categoryKey === 'all')
        return breakdown;
    const row = (breakdown.stackedChartData || []).find((r) => r.categoryKey === categoryKey);
    if (!row) {
        return {
            ...breakdown,
            stackedChartData: [],
    ...
    return {
        ...
        tableRows: (breakdown.tableRows || []).filter((r) => r.category === row.category),
        levelSummary: levelSummaryFromRow(row),
        levelTotals: {
            beginner: row.beginner || 0,
...
/** Narrow a gender × category breakdown to one course category (or return unchanged for `all`). */
/** One bar per stack segment (level or gender) for a single filtered category row. */
export function buildGroupedSegmentsFromStackRow(row, series, colorMap) {
    if (!row)
        return [];
    const total = row.total || 0;
    return series.map(({ countKey, name }) => {
        ...
        return {
            ...
            count,
            total,
            fill: colorMap[countKey],
            chartLabel: count > 0 ? formatCountWithPercent(count, total, 1) : '',
        };
    }).filter((d) => d.count > 0);
}

/**
 * Gender card when one course category is selected.
 * `all` → stacked male/female/other per participant level; otherwise one bar per level for that gender.
 */
export function buildGenderLevelChartForCategory(row, genderFilter = 'all') {
    if (!row) {
        return {
            mode: 'empty',
            chartData: [],
            total: 0,
            genderFilter,
            genderOptions: [{ value: 'all', label: 'All genders' }],
        };
    }
    ...
    const genderOptions = [
        ...
        ...GENDER_STACK_KEYS
            ...
            .map((key) => ({ value: key, label: GENDER_LABEL_BY_KEY[key] })),
    ];

    if (genderFilter && genderFilter !== 'all') {
        const genderTotal = row[genderFilter] || 0;
        const levelSegments = [
            ...TRAINEE_LEVELS.map((level) => ({
                segment: level.label,
                countKey: level.key,
                count: row[`${genderFilter}_${level.key}`] || 0,
            })),
            {
                segment: 'Unspecified',
                countKey: 'unspecified',
                count: row[`${genderFilter}_unspecified`] || 0,
            },
        ];
        const chartData = levelSegments
            .filter((d) => d.count > 0)
            .map((d) => ({
                ...d,
                total: genderTotal,
                fill: LEVEL_CHART_COLORS[d.countKey],
                chartLabel: formatCountWithPercent(d.count, genderTotal, 1),
            }));
        ...
        return {
            ...
            total: genderTotal,
            genderFilter,
            genderLabel: GENDER_LABEL_BY_KEY[genderFilter] || genderFilter,
            genderOptions,
        };
    }
    ...
    const levelRows = [
        ...TRAINEE_LEVELS.map((level) => {
            const male = row[`male_${level.key}`] || 0;
            const female = row[`female_${level.key}`] || 0;
            const other = row[`other_${level.key}`] || 0;
            const levelTotal = male + female + other;
            return {
                level: level.label,
                levelKey: level.key,
                male,
    ...
    const uncMale = row.male_unspecified || 0;
    const uncFemale = row.female_unspecified || 0;
    const uncOther = row.other_unspecified || 0;
    const uncTotal = uncMale + uncFemale + uncOther;
    if (uncTotal > 0) {
        levelRows.push({
            level: 'Unspecified',
            levelKey: 'unspecified',
            male: uncMale,
            female: uncFemale,
            other: uncOther,
            total: uncTotal,
    ...
    return {
        mode: 'stacked-by-level',
        chartData: levelRows.filter((d) => d.total > 0),
        total: row.total || 0,
        genderFilter: 'all',
        genderOptions,
    };
...
export function filterGenderBreakdownByCategory(breakdown, categoryKey) {
    if (!breakdown || !categoryKey || categoryKey === 'all')
        return breakdown;
    const row = (breakdown.stackedChartData || []).find((r) => r.categoryKey === categoryKey);
    if (!row) {
        return {
            ...
            tableRows: [],
            genderSummary: [],
            genderTotals: emptyGenderTotals(),
            grandTotal: 0,
            categoryFilterActive: true,
            selectedCategoryKey: categoryKey,
            selectedCategory: '',
    ...
    return {
        ...
        stackedChartData: [row],
        tableRows: (breakdown.tableRows || []).filter((r) => r.category === row.category),
        genderSummary: genderSummaryFromRow(row),
        genderTotals: {
            male: row.male || 0,
            female: row.female || 0,
            other: row.other || 0,
 ...
 */
export function buildGenderByCourseCategory(trainees, programById, options = {}) {
    const matrix = {};
    RENEWABLE_ENERGY_COURSES.forEach((c) => {
        matrix[c.key] = emptyLevelGenderRow();
    });
    matrix._uncategorized = emptyLevelGenderRow();
    const byLabel = {};
    ...
    rows.forEach((trainee) => {
        const catKey = resolveTraineeCourseCategoryKey(trainee, programById, trainee.program_id);
        const categoryLabel = getCourseLabel(getCourseKeyFromProgram(programById[trainee.program_id]))
            || getTraineeCourseLabel(trainee, programById);
        const genderKey = normalizeParticipantGenderKey(trainee);
        const rawLevel = getTraineeLevelFromDoc(trainee);
        const levelKey = LEVEL_STACK_KEYS.includes(rawLevel) ? rawLevel : 'unspecified';
        const levelGenderKey = `${genderKey}_${levelKey}`;
        const bucket = matrix[catKey] ?? matrix._uncategorized;
        bucket[genderKey]++;
        bucket[levelGenderKey] = (bucket[levelGenderKey] || 0) + 1;
        if (!byLabel[categoryLabel]) {
            byLabel[categoryLabel] = emptyLevelGenderRow();
        }
        byLabel[categoryLabel][genderKey]++;
        byLabel[categoryLabel][levelGenderKey] = (byLabel[categoryLabel][levelGenderKey] || 0) + 1;
