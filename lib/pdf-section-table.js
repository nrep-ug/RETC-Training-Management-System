...
export function appendGenderReportSections(doc, startY, {
    ...
    enrollmentRowsAlreadyExpanded = false,
}) {
    let y = startY;
    if (!trainees?.length)
        return y;

    const breakdown = buildGenderByCourseCategory(trainees, programById, {
        alreadyExpanded: enrollmentRowsAlreadyExpanded,
    });
